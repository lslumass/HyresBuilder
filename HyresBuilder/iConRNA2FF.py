...
def iConRNA2System(psf, system, ffs):
    ...
    # base stacking
    Aform = CustomCentroidBondForce(2, '0.5*eps_stack*((ra/r)^10-2*(ra/r)^5)*sr; sr=1/(1+exp(50*(r-rs))); r=distance(g1, g2)')
    Aform.setName('IntraStackingForce')
    Aform.addPerBondParameter('eps_stack')
    Aform.addGlobalParameter('ra', 0.40*unit.nanometers)
    Aform.addGlobalParameter('rs', 0.50*unit.nanometers)    # for cutoff and switch
    ...
    # base stacking
    fstack = CustomCentroidBondForce(2, '0.5*eps_stack*((r1/r)^10-2*(r1/r)^5)*sr; sr=1/(1+exp(50*(r-rs))); r=distance(g1, g2)')
