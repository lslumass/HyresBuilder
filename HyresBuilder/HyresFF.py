...
def HyresRNASystem(psf, system, ffs):
    ...
    formula = f"""(ke/20.0*charge1*charge2)/r*exp(-r/dh);
                  ...
                  dh={dh.value_in_unit(unit.angstrom)}
               """
    CNBForce = CustomNonbondedForce(formula)
...
def HyresTestSystem(psf, system, ffs):
    top = psf.topology
    # 2) constructe the force field
    print('\n################# constructe the HyRes force field ####################')
    # get nonbonded force
...
def TestRNASystem(psf, system, ffs):
    ...
    # base stacking for neighbouring bases
    fstack = CustomCentroidBondForce(2, "eps_stack*(5*(r0/r)^10-6.0*(r0/r)^6); r=distance(g1,g2);")
    fstack.setName('StackingForce')
    fstack.addPerBondParameter('eps_stack')
    fstack.addGlobalParameter('r0', 0.34*unit.nanometers)
    ...
    stackGen.addGlobalParameter('raa', 0.46*unit.nanometers)
    stackGen.setCutoffDistance(0.75*unit.nanometers)
    for idx in range(len(NAs)):
