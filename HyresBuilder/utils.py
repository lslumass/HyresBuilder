...
def load_ff(model='protein'):
    if model == 'protein':
        ...
        path1 = pkg_res.resource_filename("HyresBuilder", "forcefield/top_hyres_mix2.inp")
        path2 = pkg_res.resource_filename("HyresBuilder", "forcefield/param_hyres_mix2.inp")
...
def setup(model, args, dt, pressure=1*unit.atmosphere, friction=0.1/unit.picosecond, gpu_id="0"):
    ...
    ffs = {
        ...
        'eps_hb': 2.5*unit.kilocalorie_per_mole,                    # hydrogen bond strength
        'sigma_hb': 0.29*unit.nanometer,                            # sigma of hydrogen bond
        'eps_base': 2.2*unit.kilocalorie_per_mole,                 # base stacking strength
    }
...
def setup2(model, args, dt, pressure=1*unit.atmosphere, friction=0.1/unit.picosecond, gpu_id="0"):
    ...
    if model == 'protein':
        ...
        top_RNA, param_RNA = load_ff('RNA_mix')
        top_pro, param_pro = load_ff('protein_mix2')
        params = CharmmParameterSet(top_RNA, param_RNA, top_pro, param_pro)
    else:
        print("Error: Only 'protein', 'RNA', and 'mix' models are supported.")
        exit(1)
...
class Test:
    def __init__(self, eps_hb=1.80, eps_base=2.20):
        self.eps_hb = eps_hb
    ...
    def setup(self, model, args, dt, pressure=1*unit.atmosphere, friction=0.1/unit.picosecond, gpu_id="0"):
        ...
        ffs = {
            ...
            'ke': 138.935456,                                           # Coulomb constant, ONE_4PI_EPS0
            'er': er_t*61.0/77.6,                                         # relative dielectric constant
            'eps_hb': self.eps_hb*unit.kilocalorie_per_mole,                    # hydrogen bond strength
            'sigma_hb': 0.29*unit.nanometer,                            # sigma of hydrogen bond
            'eps_base': self.eps_base*unit.kilocalorie_per_mole,                 # base stacking strength
        }
