...
def setup(model, args, dt, pressure=1*unit.atmosphere, friction=0.1/unit.picosecond, gpu_id="0"):
    ...
    print('relative electric constant: ', er_t)       
...
def setup2(model, args, dt, eps_hb=2.0, eps_base=2.20, pressure=1*unit.atmosphere, friction=0.1/unit.picosecond, gpu_id="0"):
    # model = 'protein', 'RNA', 'mix'
    # input parameters
    pdb_file = args.pdb
    psf_file = args.psf
    ...
    ffs = {
        ...
        'ke': 138.935456,                                           # Coulomb constant, ONE_4PI_EPS0
        'er': er_t,                                                  # relative dielectric constant
        'eps_hb': eps_hb*unit.kilocalorie_per_mole,                    # hydrogen bond strength
        'sigma_hb': 0.29*unit.nanometer,                            # sigma of hydrogen bond
        'eps_base': eps_base*unit.kilocalorie_per_mole,                 # base stacking strength
    }
    # 0) load force field files
    if model == 'protein':
        top_pro, param_pro = load_ff('protein')
